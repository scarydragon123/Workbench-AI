...
import { Component, Location as LocationType, ProjectSuggestion, InventoryItem, Project, ProjectComponent } from './types.js';
import { useInventory } from './context.js';
...
export const CameraIcon = () => <svg {...iconProps} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z" /><circle cx="12" cy="13" r="3" /></svg>;
...
export const LightbulbIcon = () => <svg {...iconProps} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5" /><path strokeLinecap="round" strokeLinejoin="round" d="M9 18h6" /><path strokeLinecap="round" strokeLinejoin="round" d="M10 22h4" /></svg>;
...
export const SettingsIcon = () => <svg {...iconProps} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 0 2l-.15.08a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l-.22-.38a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1 0-2l.15-.08a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z" /><circle cx="12" cy="12" r="3" /></svg>;
...
export const SecondaryButton: React.FC<ButtonProps> = ({ children, className = '', ...props }) => (
    <button
      className={`bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded-lg shadow-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-75 transition-all dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 dark:focus:ring-gray-500 disabled:bg-gray-100 disabled:text-gray-400 dark:disabled:bg-gray-800 dark:disabled:text-gray-500 disabled:cursor-not-allowed ${className}`}
      {...props}
    >
      {children}
    </button>
...
export const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, size = 'md' }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-70 z-50 flex justify-center items-center p-4 animate-fade-in" onClick={onClose} role="dialog" aria-modal="true">
      <div 
        ref={modalRef}
      ...
      >
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-teal-500 dark:text-teal-400">{title}</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors" aria-label="Close modal"><XIcon /></button>
        </div>
...
export const ComponentCard: React.FC<ComponentCardProps> = ({ component, quantity, location, onClick }) => (
    <div 
        ...
        onClick={onClick}
    >
        <h3 className="font-bold text-lg text-gray-800 dark:text-gray-100 truncate group-hover:text-teal-500 dark:group-hover:text-teal-400">{component.name}</h3>
        ...
        <div className="mt-4 flex justify-between items-end">
            ...
            <div>
                ...
                <p className="font-bold text-2xl text-teal-500 dark:text-teal-400">{quantity}</p>
            </div>
        </div>
    </div>
);
...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, onClick }) => (
    <div 
        ...
        onClick={onClick}
    >
        <h3 className="font-bold text-lg text-teal-500 dark:text-teal-400">{project.name}</h3>
            ...
            <ul className="space-y-1 text-sm">
                {project.components.map((c, i) => (
                    <li key={i} className="flex justify-between items-center">
                        ...
                        {c.available ? 
                            <span className="text-xs bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300 px-2 py-0.5 rounded-full">Available</span> : 
                            <span className="text-xs bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300 px-2 py-0.5 rounded-full">Missing</span>}
                    </li>
                ))}
            </ul>
        </div>
                    ...
                    {project.components.map((c, i) => {
                         ...
                         return (
                            <li key={i} className="flex justify-between items-center text-gray-700 dark:text-gray-300">
                                ...
                                <span className={`font-mono text-xs ${isAvailable ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Component Details" size="xl">
            <div className="space-y-6">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-gray-700 dark:text-gray-300 mb-4">{component.description}</p>
                         <div className="flex flex-wrap gap-2">
                            {component.tags.map(tag => <span key={tag} className="bg-gray-200 text-teal-700 dark:bg-gray-700 dark:text-teal-300 text-xs font-semibold px-2 py-1 rounded-full">{tag}</span>)}
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
                   <button onClick={handleDelete} className="flex items-center gap-1 text-sm text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                     ...
                     <div>
                        ...
                        <select id="comp-loc" value={locationId} onChange={e => setLocationId(e.target.value)} className="input-style h-full" required disabled={locations.length === 0}>
                             {locations.length > 0 ? (
                                locations.map(loc => <option key={loc.id} value={loc.id}>{loc.name}</option>)
                             ) : (
                                <option>Please add a location first</option>
                             )}
                        </select>
                    </div>
                </div>
                <div className="flex justify-end gap-4 pt-4">
                    ...
                    <Button type="submit" disabled={locations.length === 0}>Add Component</Button>
                </div>
            </form>
        </Modal>
    );
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Create New Project" size="lg">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="proj-desc" value={description} onChange={e => setDescription(e.target.value)} className="input-style" rows={2}></textarea>
                </div>
                
                <div className="space-y-2">
                    <label className="label-style">Components</label>
                    <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                        {projectComponents.map(pc => {
                            const component = components.find(c => c.id === pc.componentId);
                            return (
                                <div key={pc.componentId} className="flex justify-between items-center bg-gray-100 dark:bg-gray-700/50 p-2 rounded-md">
                                    <span>{pc.quantity}x {component?.name || 'Unknown'}</span>
                                    <button type="button" onClick={() => handleRemoveComponent(pc.componentId)} className="text-red-500 hover:text-red-700"><TrashIcon /></button>
                                </div>
                            )
                        })}
                    </div>
                     <div className="flex items-end gap-2 pt-2">
                        <div className="flex-grow">
                            ...
                            <select id="select-comp" value={selectedComponentId} onChange={e => setSelectedComponentId(e.target.value)} className="input-style">
                         ...
                         <div>
                            ...
                            <input id="select-qty" type="number" min="1" value={selectedQuantity} onChange={e => setSelectedQuantity(Number(e.target.value))} className="input-style w-20"/>
                        </div>
                        <SecondaryButton type="button" onClick={handleAddComponent} className="h-10"><PlusIcon/></SecondaryButton>
                    </div>
                </div>
